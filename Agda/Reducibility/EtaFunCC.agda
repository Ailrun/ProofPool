...
module Reducibility.EtaFunCC where
...
module LogRel where
  ...
  infix 4 _↠ᶜ_
  data _↠ᶜ_ : Cont∧ Γ A B → Cont∧ Γ A B → Set where
    `∧c : ------------------------------------------------------------
          N ∷ L ∷ K ↠ᶜ `let N `in ext[ qᵉ qᵉ Wkᵉ (_ ∷ _ ∷ []) ] L ∷ K
  ...
  ξ-of-↝*-↠ᶜ* : ∀ {ℓ″} (_↝_ : Rel (Tm Γ A) ℓ″) (f : Tm Γ A → Cont∧ Δ B C) →
                (∀ {L L′} → L ↝ L′ → f L ↠ᶜ f L′) →
                ∀ {L L′} → Star _↝_ L L′ → f L ↠ᶜ* f L′
  ξ-of-↝*-↠ᶜ* _↝_ f ξ-rule ε           = ε
  ξ-of-↝*-↠ᶜ* _↝_ f ξ-rule (L↠ ◅ L′↠*) = ξ-rule L↠ ◅ ξ-of-↝*-↠ᶜ* _↝_ f ξ-rule L′↠*

  ξ-of-↠ᶜ* : ∀ (f : Cont∧ Γ A B → Cont∧ Δ C D) →
             (∀ {L L′} → L ↠ᶜ L′ → f L ↠ᶜ f L′) →
             ∀ {L L′} → L ↠ᶜ* L′ → f L ↠ᶜ* f L′
  ξ-of-↠ᶜ* f ξ-rule ε           = ε
  ξ-of-↠ᶜ* f ξ-rule (L↠ ◅ L′↠*) = ξ-rule L↠ ◅ ξ-of-↠ᶜ* f ξ-rule L′↠*
...
module LogRelProp where
  ...
  haltsᶜ-closed : K′ haltsᶜ → K ↠ᶜ K′ → K haltsᶜ
  haltsᶜ-closed (_ , K′↠* , V) K↠K′ = -, K↠K′ ◅ K′↠* , V

  ext[-]-preserves-halts : ∀ (δ : Ext Γ Δ) → M halts → ext[ δ ] M halts
  ext[-]-preserves-halts δ (_ , M↠* , V) = -, ext[ δ ]↠* M↠* , ext[-]-preserves-Nf δ V

  ext[-]ᶜ-preserves-Nfᶜ : ∀ (δ : Ext Δ Γ) →
                          Nfᶜ K →
                          Nfᶜ (ext[ δ ]ᶜ K)
  ext[-]ᶜ-preserves-Nfᶜ δ []      = []
  ext[-]ᶜ-preserves-Nfᶜ δ (V ∷[]) = ext[-]-preserves-Nf (qᵉ qᵉ δ) V ∷[]
  ...
  (`∧c {K = K})           ↠ᶜ`$$ᶜ M = K `$$ᶜ↠ `∧c ◅ ε
  []                      ↠ᶜ`$$ᶜ M = ε
  (_∷_ {K = K} N↠N′ K↠K′) ↠ᶜ`$$ᶜ M = (K `$$ᶜ↠ (`let ↠-refl `in N↠N′)) ◅ (K↠K′ ↠ᶜ`$$ᶜ (`let M `in _))

  _↠ᶜ*`$$ᶜ_ : K ↠ᶜ* K′ →
              ∀ (M : Tm Γ A) →
              K `$$ᶜ M ↠* K′ `$$ᶜ M
  ε               ↠ᶜ*`$$ᶜ M = ε
  (K↠K′ ◅ K′↠*K″) ↠ᶜ*`$$ᶜ M = K↠K′ ↠ᶜ`$$ᶜ M ◅◅ K′↠*K″ ↠ᶜ*`$$ᶜ M
  ...
  ↠ᶜ-≡-trans : K ↠ᶜ K′ →
               K′ ≡ K″ →
               K ↠ᶜ K″
  ↠ᶜ-≡-trans {K = K} K↠ eq = subst (K ↠ᶜ_) eq K↠

  infixr 30 ext[_]ᶜ↠ᶜ_
  ext[_]ᶜ↠ᶜ_ : ∀ (δ : Ext Γ Δ) → K ↠ᶜ K′ → ext[ δ ]ᶜ K ↠ᶜ ext[ δ ]ᶜ K′
  ext[ δ ]ᶜ↠ᶜ (`∧c {L = L} {K = K}) =
    ↠ᶜ-≡-trans
      `∧c
      (cong
        ...
        (cong
          ...
          (ext[-]-ext[-]-ext[-]-ext[-]-cong
            ...
            L)))
  ext[ δ ]ᶜ↠ᶜ []            = []
  ext[ δ ]ᶜ↠ᶜ (N↠N′ ∷ K↠K′) = ext[ qᵉ qᵉ δ ]↠ N↠N′ ∷ ext[ δ ]ᶜ↠ᶜ K↠K′

  infixr 30 ext[_]ᶜ↠ᶜ*_
  ext[_]ᶜ↠ᶜ*_ : ∀ (δ : Ext Γ Δ) → K ↠ᶜ* K′ → ext[ δ ]ᶜ K ↠ᶜ* ext[ δ ]ᶜ K′
  ext[ δ ]ᶜ↠ᶜ* ε               = ε
  ext[ δ ]ᶜ↠ᶜ* (K↠K′ ◅ K′↠*K″) = ext[ δ ]ᶜ↠ᶜ K↠K′ ◅ ext[ δ ]ᶜ↠ᶜ* K′↠*K″

  ext[-]ᶜ-preserves-haltsᶜ : ∀ (δ : Ext Γ Δ) → K haltsᶜ → ext[ δ ]ᶜ K haltsᶜ
  ext[-]ᶜ-preserves-haltsᶜ δ (_ , K↠* , V) = -, ext[ δ ]ᶜ↠ᶜ* K↠* , ext[-]ᶜ-preserves-Nfᶜ δ V
  ...
  ℜᶜ-closed : ℜᶜ[ A₁ & A₂ ] K′ → K ↠ᶜ K′ → ℜᶜ[ A₁ & A₂ ] K
  ℜᶜ-closed {K′ = K′} {K = K} rK K↠K′ = haltsᶜ-closed {K′ = K′} {K = K} (rK .proj₁) K↠K′ , λ δ rM₁ rM₂ → halts-closed* (rK .proj₂ δ rM₁ rM₂) ((ext[ δ ]ᶜ↠ᶜ K↠K′) ↠ᶜ`$$ᶜ _)
  ...
  ext[-]-preserves-ℜᶜ⁻ {K = []}    δ rK γ = rK (γ ∘ δ)
  ...
  ext[-]-preserves-ℜᶜ : ∀ (δ : Ext Γ Δ) → ℜᶜ[ A₁ & A₂ ] K → ℜᶜ[ A₁ & A₂ ] (ext[ δ ]ᶜ K)
  ext[-]-preserves-ℜᶜ {K = K} δ rK = ext[-]ᶜ-preserves-haltsᶜ δ (rK .proj₁) , ext[-]-preserves-ℜᶜ⁻ {K = K} δ (rK .proj₂)
  ...
  mutual
    ...
    reflect {A = A₁ `∧ A₂} RM δ K rK
      with _ , K↠* , VK′ ← rK .proj₁
        with VK′
    ...    | []      = -, K↠* ↠ᶜ*`$$ᶜ _ , `↑ ext[-]-preserves-Ne δ RM
    ...    | VN′ ∷[] = -, K↠* ↠ᶜ*`$$ᶜ _ , `let ext[-]-preserves-Ne δ RM `in VN′
  ...
  eval {σ = σ} rσ (M₁ `, M₂) δ K rK
    with rM₁ ← eval rσ M₁
       | rM₂ ← eval rσ M₂
      with rM₁M₂ ← rK .proj₂ Idᵉ (ext[-]-preserves-ℜ δ rM₁) (ext[-]-preserves-ℜ δ rM₂)
